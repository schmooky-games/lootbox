...
from src.lootboxes.models import LootboxResponse
...
@router.get("/lootbox/{lootbox_id}", response_model=LootboxResponse)
def get_lootbox(lootbox_id: str):
    lootbox_data = redis.get(lootbox_id)
    lootbox = LootboxResponse.model_validate_json(lootbox_data)
    return lootbox


@router.post("/deactivate_lootbox/{lootbox_id}", response_model=LootboxResponse)
...
@router.get("/lootboxes", response_model=Dict[str, LootboxResponse])
def list_lootboxes():
    all_keys = redis.keys()
    lootboxes_data = {}
